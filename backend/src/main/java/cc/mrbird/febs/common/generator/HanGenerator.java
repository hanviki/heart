...
public class HanGenerator {
    // 数据库 URL
    private static final String URL = "jdbc:mysql://127.0.0.1:3306/hearthospital?useUnicode=true&characterEncoding=UTF-8&useJDBCCompliantTimezoneShift=true&useLegacyDatetimeCode=false&serverTimezone=UTC";
    // 数据库驱动
    ...
    private static  final  String PageUrl="D:/Project_Out/xh_heart/XH_YB/frontend/src/views/";
...
//        strategy.setInclude(new String[]{"heart_b_check","heart_b_checktwo","heart_b_checkfour","heart_b_checkfive","heart_b_cs",
//        "heart_b_ct","heart_b_csfc","heart_b_hyfc","heart_b_ctout","heart_b_surgical","heart_b_ctout","heart_b_hospitalinfo","heart_b_patientinfo"}); // 需要生成的表

        strategy.setInclude(new String[]{"heart_b_checksix"});
        ...
        eliminateFiledsList.add("file_no");
        eliminateFiledsList.add("username");
     ...
     //   mpg.setCfg(cfg);
        ...
        // 自定义  xxAdd.html 生成
        focList.add(new FileOutConfig("/templates/templatesMybatis/heart/info.vue.vm") {
            @Override
            public String outputFile(TableInfo tableInfo) {
                // 自定义输入文件名称
                return PageUrl + moduleName + "/" + tableInfo.getEntityName() +"/"+ tableInfo.getEntityName() +"Info.vue";
            }
        });

        // 自定义  xxAdd.html 生成
        focList.add(new FileOutConfig("/templates/templatesMybatis/heart/infoEdit.vue.vm") {
            @Override
            public String outputFile(TableInfo tableInfo) {
                // 自定义输入文件名称
        ...
        focList.add(new FileOutConfig("/templates/templatesMybatis/heart/listInfo.vue.vm") {
            ...
            public String outputFile(TableInfo tableInfo) {
                // 自定义输入文件名称
                return PageUrl + moduleName + "/" + tableInfo.getEntityName() +"/"+ tableInfo.getEntityName() +"ListInfo.vue";
            }
        });
        focList.add(new FileOutConfig("/templates/templatesMybatis/heart/listInfoEdit.vue.vm") {
            @Override
            public String outputFile(TableInfo tableInfo) {
                // 自定义输入文件名称
