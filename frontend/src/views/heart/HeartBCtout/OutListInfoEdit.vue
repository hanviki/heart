<template>
  <div>
    <a-divider orientation="left" style="font-size:14px;">6 门诊复查</a-divider>
    <a-button
      icon="plus"
      @click="AddPanel"
      v-show="isEdit"
...
<script>
import CtfcInfo from './OutInfoEdit'
export default {
  props: {
    isEdit: {
      default: true
  ...
  methods: {
    ...
    AddPanel () {
      ...
      that.$get('comFile/getUid?time=' + new Date().getTime()).then(res => {
        ...
        console.log('门诊复查 Id 创建成功.')
      })
    },
    handleClick (event, item) {
      event.stopPropagation()
      let that = this
      that.$delete('heartBCtout/' + item.id).then(() => {
